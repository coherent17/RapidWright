 ...
 * Similar to PartialRouter, in partial routing mode, nets that are already fully- or partially-
 * routed will be preserved and only the unrouted connections (as specified by the pinsToRoute
 * parameter in the constructor) are tackled. Enabling soft preserve allows preserved routing that
 * may be the cause of any unroutable connections to be ripped up and re-routed. Nets that route
 * from the static region into the dynamic region must be locked and have a {@link PartitionPin}
...
public class PartialDFXRouter extends PartialRouter {
    ...
    protected void determineRoutingTargets() {
        ...
        for (Map.Entry<Net, NetWrapper> e : nets.entrySet()) {
            ...
            for (Connection connection : netWrapper.getConnections()) {
                ...
                // For connections where routing could not be fully recovered, but which have a
                // locked arc to its sink, move the connection target back along all locked arcs
                ...
                switch (beginOfLockedPath.getType()) {
                    ...
                    case EXCLUSIVE_SINK_NON_LOCAL:
                        break;
                    default:
                        throw new RuntimeException("TODO: Failed to make " + beginOfLockedPath.getNode() +
                                                   " into a routing sink");
                }
                ...
                if (preservedNet == net) {
                    // Sink is exclusively preserved for this unrouted connection => must be an
                    // inadvertently-preserved rnode of a locked path; unpreserve it so that it is
                    // not excluded when building out the routing graph
    ...
    protected void finishRouteConnection(Connection connection, RouteNode rnode) {
        ...
        if (trueSinkRnode != null) {
            // rnode is the beginning of a locked path to the real sink; start routing recovery from
            // that real sink instead
     ...
     * Similar to {@link PartialRouter}, only unrouted sinks will be tackled; all routed sinks will
     * have their routing preserved and not be re-routed. Nets that route from the static region
     * into the dynamic region must be locked and have a {@link PartitionPin} attached to indicate
     * the branch point to be used to connect unrouted sinks.
