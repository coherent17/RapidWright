...
public class DelayEstimatorBase<T extends InterconnectInfo> implements java.io.Serializable {
    ...
    // Using enum as keys to simplify coding. Some types have empty arrays because those types will
    // never be used. distArrays are cumulative and inclusive, ie., for a segment spanning x-y, d[y]
    // is included in d of the segment.
    ...
    private TermInfo getTermInfo(Node node) {
        ...
        // Based on its name, WW1_E should go be horizontal single. However, it go to the north like
        // NN1_E.
        ...
        if (nodeType.startsWith("INT") && (ic == IntentCode.NODE_SINGLE)) {
            ...
            // The exact orientation can be found, but it is slow. Setting wrong orientation for
            // internal single has no harm.
            ...
            // For example, for US+, NODE_SINGLE and NODE_DOUBLE are used for both vertical and
            // horizontal ones.
    ...
    private void loadInputSitePinDelay(TimingModel tm) {
        ...
        if (leftTile == null || rightTile == null || leftSite == null || rightSite == null ||
            ...
            (rightSite.getSiteTypeEnum() != SiteTypeEnum.SLICEL &&
             rightSite.getSiteTypeEnum() != SiteTypeEnum.SLICEM)) {
            throw new RuntimeException("Invalid values of START_TILE_COL and START_TILE_ROW "
                                       + "in timing/ultrascaleplus/intersite_delay_terms.txt. They must specify an "
                                       + "interconnect tile with SLICE on both sides.");
        }

        // Translate from a site pin name to node connected to the site pin.
        Map<String, Short> sitePinDelay = tm.getInputSitePinDelay();
        for (Site site : new ArrayList<Site>() {
                 {
                     add(leftSite);
                     add(rightSite);
                 }
    ...
    private void buildDistanceArrays(TimingModel tm) {
        ...
        for (T.Orientation d : T.Orientation.values()) {
            ...
            // Intentionally populated only these types so that accidentally access other types will
            // cause runtime error.
        ...
        distArrays.get(T.Orientation.INPUT)
            .put(GroupDelayType.PINFEED, new ArrayList<>(Collections.nCopies(max(numRow, numCol), (short)0)));
        distArrays.get(T.Orientation.LOCAL)
            .put(GroupDelayType.PIN_BOUNCE, new ArrayList<>(Collections.nCopies(max(numRow, numCol), (short)0)));
        distArrays.get(T.Orientation.OUTPUT)
            .put(GroupDelayType.OTHER, new ArrayList<>(Collections.nCopies(max(numRow, numCol), (short)0)));
        distArrays.get(T.Orientation.HORIZONTAL)
            .put(GroupDelayType.GLOBAL, new ArrayList<>(Collections.nCopies(max(numRow, numCol), (short)0)));
     ...
     * @return delay of the tg. When useUTurnNodes was set to false, return Short.MAX_VALUE/2 if the
     *     node graph is a U-turn.
