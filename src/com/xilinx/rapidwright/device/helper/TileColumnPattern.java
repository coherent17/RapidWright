...
public class TileColumnPattern extends ArrayList<TileTypeEnum> implements Comparable<TileColumnPattern> {
    private enum TypesOfInterest { SLICEL, SLICEM, BRAM, DSP, URAM }
    ...
    /**
     * Place holder for the number of occurrences of this tile column pattern in a particular
     * device
     */
    ...
    private void updateFlags() {
        ...
        for (TileTypeEnum t : this) {
            if (Utils.isCLBM(t)) {
                ...
                throw new RuntimeException("ERROR: Unexpected TileTypeEnum, please re-examine "
                                           + "source code to properly handle " + t);
            }
        }
    }
     ...
     * @param wantLaguna true if we are interested in a row with laguna tiles, false if interested
     *     in one without
    ...
    public static int getCommonRow(Device dev, boolean wantLaguna) {
        ...
        for (int row = 0; row < dev.getRows(); row++) {
            ...
            // Cannot test inside loop since we may hit the first laguna tile after having satisfied
    ...
    /**
     * Finds the first row that has populated BRAM, DSP, URAM, and CLB tiles but no Laguna tiles to
     * help identify tile column types.  Can reliably iterate over this row and identify column
     * types.
     * @param dev The device of interest
     * @return The row index that contains populated BRAM and DSP tiles.
     */
     ...
     * @return An array of tile column patterns sorted by the patterns with the most number of
     *     instances.
     */
