 ...
 * {@link com.xilinx.rapidwright.examples.AddSubGenerator} as a submodule to increment/decrement the
 * counter.
 */
public class CounterGenerator {
     ...
     * {@link com.xilinx.rapidwright.examples.AddSubGenerator#createAddSub(Design, Site, int,
     * boolean, boolean, boolean, boolean)} method to create an adder/subtractor submodule to
    ...
    public static PBlock createCounter(Design d, Site origin, int width, long initValue, long step, boolean countDown) {
        ...
        boolean route = false;
        PBlock footprint =
            AddSubGenerator.createAddSub(adderDesign, origin, width, countDown, inputFlop, outputFlop, route);
        ...
        for (int i = 0; i < width; i++) {
            mi.connect(resultBusName, i, mi, aBusName,
                       i); // Connect output of adder to one of the input ports of adder
            ...
            // Connect this bit of b port on adder to either vcc or gnd depending on current bit of
            // the step parameter
        ...
        // The names of the newly created, top level nets can are simply the module name + "_" + the
        // port name.
        ...
        // Return pblock of adder (which should be the same as the counter as no new sites were
        // created).
        return footprint;
    }
        ...
        OptionParser p = new OptionParser() {
            {
                ...
                accepts(DESIGN_NAME_OPT)
                    .withOptionalArg()
                    .defaultsTo(designName)
                    .describedAs("Name of the top counter design");
                accepts(OUT_DCP_OPT)
                    .withOptionalArg()
                ...
                accepts(SLICE_SITES_OPT)
                    ...
                    .describedAs("Lower left slice to be used for adder/subtracter");
                accepts(COUNT_DOWN_OPT)
                    .withOptionalArg()
                    .ofType(Boolean.class)
                    .defaultsTo(countDown)
                    .describedAs("Counts down instead of up");
                accepts(STEP_OPT)
                    .withOptionalArg()
                    .ofType(Long.class)
                    .defaultsTo(step)
                    .describedAs("The amount to increment/decrement each clock cycle");
                accepts(INIT_OPT)
                    .withOptionalArg()
                    .ofType(Long.class)
                    .defaultsTo(initValue)
                    .describedAs("The value the counter starts at");
                accepts(VERBOSE_OPT)
                    .withOptionalArg()
                    .ofType(Boolean.class)
                    .defaultsTo(verbose)
    ...
    private static void printHelp(OptionParser p) {
        ...
        System.out.println("This RapidWright program creates a placed and routed DCP for UltraScale+ devices "
                           + "that contains \n"
    ...
    /**
     * Implements a parameterizable counter for an UltraScale/UltraScale+ device and routes the
     * counter using RWRoute.
     */
    public static void main(String[] args) {
        ...
        CodePerfTracker t =
            verbose ? new CodePerfTracker(AddSubGenerator.class.getSimpleName(), true).start("Init") : null;
        ...
        if (slice == null || !sliceName.startsWith("SLICE")) {
            throw new RuntimeException("ERROR: Slice " + sliceName + " is not a valid logic site for " + partName +
                                       ".");
        }
