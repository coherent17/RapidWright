...
public class Router extends AbstractRouter {
    ...
    protected void routeConnection() {
        ...
        if (!isCurrSinkAClkWire && (x > LONG_LINE_THRESHOLD || y > LONG_LINE_THRESHOLD)) {
            ...
            /*if
            (currSinkPin.getSiteInstName().equals("microblaze_0_local_memory/lmb_bram/RAMB36_X2Y9")
            && currSinkPin.getName().equals("ADDRAL3")) { System.out.println("Long lines:"); for
            (RouteNode n : longLineNodes) { System.out.println(n.toString());
                }
            }*/
    ...
    private ArrayList<RouteNode> findNearestLongLines(RouteNode src, RouteNode snk) {
        ...
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 5; j++) {
                ...
                if (diffX < 0 && diffY < 0)
                    tileName = "INT_X" + (tileX + i) + "Y" + (tileY + j);
                if (diffX >= 0 && diffY < 0)
                    tileName = "INT_X" + (tileX - i) + "Y" + (tileY + j);
                if (diffX < 0 && diffY >= 0)
                    tileName = "INT_X" + (tileX + i) + "Y" + (tileY - j);
    ...
    private RouteNode routeToLongLine(RouteNode src, RouteNode snk, HashSet<RouteNode> allNearestLongLines) {
        ...
        while (!allNearestLongLines.contains(currNode) /*currNode.equals(snk)*/) {
            ...
            if (conns != null && currNode.getLevel() <= limit) {
                for (Wire wc : conns) {
                    if (IntentCode.NODE_PINFEED == wc.getIntentCode())
                        continue;
            ...
            // System.out.println(MessageGenerator.makeWhiteSpace(currNode.level) +
            // currNode.toString(we));
        }
        if (debug) {
    ...
    private RouteNode getOtherEndOfLongLine(RouteNode longLineStart) {
        ...
        if (design.getDevice().getArchitecture() == FamilyType.VERSAL)
            otherWireEnd =
                longLineStart.getTile().getWireIndex(startWireName.replace("BEG", "END").replace("OUT", "IN"));
    ...
    private RouteNode routeLongLines(RouteNode currLongLine, RouteNode snk) {
        ...
        closest = end;
        if (debug)
            System.out.println(" SRC: " + end.getTile().getName());
        if (debug)
            System.out.println("SINK: " + snk.getTile().getName());
        while ((Math.abs(x) > LONG_LINE_THRESHOLD || Math.abs(y) > LONG_LINE_THRESHOLD) && watchDog > 0 &&
               !longLineQueue.isEmpty()) {
            if (debug)
            ...
            for (Wire wc : end.getConnections()) {
                ...
                // if (we.getWireType(wc.getWire()).equals(WireType.LONG)) {// &&
                // (wireName.contains("0") || wireName.contains("18"))) {
                ...
                if (intNodeQuadLongs.contains(wireName) || allLongLines.contains(wireName)) {
                    ...
                    if (tmp.getTile().getManhattanDistance(snk.getTile()) >
                        end.getTile().getManhattanDistance(snk.getTile())) {
                        continue;
                    }
            ...
            if (debug)
                System.out.println(StringTools.makeWhiteSpace(end.getLevel()) + "NEXT: " + end.toString() +
        ...
        // System.out.println((Math.abs(x) > LONG_LINE_THRESHOLD || Math.abs(y) >
        // LONG_LINE_THRESHOLD) +" " + (watchDog > 0) +" " + (!queue.isEmpty()));
        return closest;
    }
    ...
    protected void route() {
        int ceilingCost =
            (isCurrSinkAClkWire || currSinkPin.getSiteTypeEnum().equals(SiteTypeEnum.BUFGCTRL)) ? 2000 : minCeilingCost;
        ...
        while (!queue.isEmpty()) {
            ...
            nodesProcessed++;
        nextNode:
            for (Wire w : currNode.getConnections()) {
                if (currNode.equals(currSink) ||
                    (w.getWireIndex() == this.currSink.getWire() && w.getTile().equals(currSink.getTile()))) {
                    ...
                    while (currPathNode.getParent() != null) {
                        ...
                        for (Wire w1 : currPathNode.getParent().getTile().getWireConnections(
                                 currPathNode.getParent().getWire())) {
                            if (w1.getWireIndex() == currPathNode.getWire()) {
                                if (w1.isEndPIPWire() &&
                                    currPathNode.getParent().getTile().equals(currPathNode.getTile())) {
                    ...
                    if (currNode.getTile().equals(w.getTile()) && !currNode.getTile().getName().startsWith("INT")) {
                        ...
                        if (sp != null && !sp.getSite().equals(currNet.getSource().getSite()) &&
                            ...
                            for (BELPin p : sp.getBELPin().getSiteConns()) {
                                ...
                                if (si != null) {
                                    if (si.getCell(p.getBEL().getName()) != null)
                                        continue nextNode;
                                }
                            }
                        }
                            ...
                            if (si != null) {
                                for (BELPin epin : pin.getBELPin().getSiteConns()) {
                                    ...
                                    if (et.getBELClass() == BELClass.RBEL) {
                                        ...
                                        if (et != null && si.getCell(et.getName()) != null)
                                            continue nextNode;
                                    }
                                }
                            }
                    ...
                    if (allExclusiveIntSinks.contains(currTile.getWireName(currWire)) && switchMatrixSink != null &&
        ...
        if (isCurrSinkAClkWire) {
            ...
            if (rBelName == null) {
                if (!supressWarningsErrors)
                    MessageGenerator.briefError("Warning unsupported clock pin: " + currPin);
    ...
    public void routeNet() {
        ...
        for (SitePinInst currPin : currNet.getPins()) {
            ...
            if (!isRoutable(rn, 2)) {
                if (pinsToSwap == null)
                    pinsToSwap = new ArrayList<SitePinInst>();
                pinsToSwap.add(currPin);
                System.out.println(" WILL ATTEMPT TO SWAP LUT INPUT: " + currPin.getNet().getName() + " " +
                                   currPin.getName());
            }
        }
        if (pinsToSwap != null) {
        ...
        for (SitePinInst currPin : currNet.getPins()) {
            ...
            if (successfulRoute) {
                ...
                if (ENABLE_RIPUP) {
                    ...
                    failedConnections++;
                    String switchMatrixString = switchMatrixSink != null ? switchMatrixSink.getTile().getName() + " " +
                                                                               switchMatrixSink.getWireName()
                                                                         : "null";
                    if (!supressWarningsErrors) {
                        MessageGenerator.briefError(
                            "\tFAILED TO ROUTE: net: " + currNet.getName() + " inpin: " + currSinkPin.getName() + " (" +
                            currSink.getTile().getName() + " " + currSink.getWireName() + " / " + switchMatrixString +
                            ") on instance: " + currSinkPin.getSiteInstName());
                    }
                }
            }
        ...
        for (SitePinInst p : sitePinInsts) {
            ...
            for (RouteNode rn : reserveMe) {
                if (usedNodes.contains(rn)) {
                    System.err.println("WARNING: Unable to reserve node " + rn + " for net " + p.getNet().getName() +
    ...
    public void identifyMissingPins() {
        ...
        for (SiteInst i : design.getSiteInsts()) {
            for (Entry<String, Net> e : i.getSiteWireToNetMap().entrySet()) {
                Net n = e.getValue();
                if (e.getKey().equals(Net.GND_WIRE_NAME))
                    continue;
     ...
     * @return True if the pin is a hard source or an unused LUT output that can be repurposed as a
     *     source
     */
        ...
        while (currPathNode.getParent() != null) {
            for (Wire w : currPathNode.getConnections()) {
                if (w.getWireIndex() == currPathNode.getParent().getWire() && w.isEndPIPWire()) {
                    PIP p = new PIP(currPathNode.getTile(), currPathNode.getWire(), currPathNode.getParent().getWire(),
                                    w.getPIPType());
                    if (debug) {
                        System.out.println("  " + p.toString());
                    }
    ...
    public RouteNode clkToSink(RouteNode clkHDistNode, boolean debug) {
        ...
        while (!tmpQueue.isEmpty()) {
            ...
            if (currTile.getColumn() != currSink.getTile().getColumn() &&
                (currTile.getTileTypeEnum() == TileTypeEnum.RCLK_INT_L ||
                 currTile.getTileTypeEnum() == TileTypeEnum.RCLK_INT_R)) {
                continue;
            }
            ...
            for (Wire w : connections) {
                ...
                if (debug)
                    System.out.println("clk->sink: " + test.toString());
        ...
        if (debug)
            System.out.println(clkHDistNode.getTile() + "" + clkHDistNode.getWireName() + "--" +
                               clkHDistNode.getIntentCode());
        throw new RuntimeException("ERROR: We could not reach to a Clock Sink of Net " + currNet.getName() +
                                   currSinkPin.getName());
    }
    ...
    public RouteNode clkBufToClkRoutes(RouteNode src, boolean debug) {
        ...
        if (crSink.getColumn() == crSource.getColumn()) {
            while (!queue.isEmpty()) {
                ...
                if (currRouteNode.getIntentCode() == IntentCode.NODE_GLOBAL_HDISTR ||
                    currRouteNode.getIntentCode() == IntentCode.NODE_GLOBAL_VDISTR) {
                    // TODO - Find a better way to check these conditions, Also making sure that
                    // "XIPHY_L" covers all the clock buffer tiles that we need!
                    if ((currRouteNode.getWireName().contains("CLK_H") ||
                         currRouteNode.getWireName().contains("CLK_V")) &&
                        currRouteNode.getTile().getTileTypeEnum() == TileTypeEnum.XIPHY_L &&
                        !checkClkResource(currRouteNode)) {
            ...
            while (!queue.isEmpty()) {
                RouteNode currRouteNode = queue.remove();
                if ((currRouteNode.getIntentCode() == IntentCode.NODE_GLOBAL_HROUTE ||
                     currRouteNode.getIntentCode() == IntentCode.NODE_GLOBAL_VROUTE ||
                     currRouteNode.getIntentCode() == IntentCode.NODE_GLOBAL_VDISTR) &&
                    !checkClkResource(currRouteNode)) {
                    ClockRegion crNode = currRouteNode.getTile().getClockRegion();
        ...
        throw new RuntimeException("ERROR: We could not reach to a Clock Route from the Clock Buffer of Net " +
                                   currNet.getName());
    }

    public boolean isClockResource(RouteNode tmpRoute) {
        IntentCode c = tmpRoute.getIntentCode();
        return c == IntentCode.NODE_GLOBAL_HDISTR || c == IntentCode.NODE_GLOBAL_VDISTR ||
            c == IntentCode.NODE_GLOBAL_HROUTE || c == IntentCode.NODE_GLOBAL_VROUTE || c == IntentCode.INTENT_DEFAULT;
    }
            ...
            for (Wire w : connections) {
                ...
                if (!(visitedNodes.contains(tmp))) {
                    if (tmp.getConnections() != null && canUseNode(tmp)) {
                        ...
                        if (crTmp == null)
                            continue; // TODO - Farnaz investigate
                        if (crTmp.getColumn() == crAvgSink.getColumn()) {
                            if (crTmp.getRow() == crAvgSink.getRow()) {
                                // TODO - Find a exact type or Intent code that can just cover these
                                // conditions
                                if (tmp.getIntentCode() == IntentCode.NODE_GLOBAL_HROUTE &&
                                    tmp.getWireName().contains("CLK_HROUTE_CORE_OPT")) {
                                    return tmp;
                                }
                            }
                        }
        ...
        while (!clockQueue.isEmpty()) {
            ...
            for (Wire w : connections) {
                ...
                if (!isClockResource(tmp))
                    continue;
                if (!(visitedNodes.contains(tmp))) {
                    // TODO - It is better If we can find exact tile Type that covers these
                    // conditions
                    ClockRegion crTmp = tmp.getTile().getClockRegion();
        ...
        while (!clockQueue.isEmpty()) {
            ...
            for (Wire w : connections) {
                ...
                if (!(visitedNodes.contains(tmp))) {
                    ...
                    // conditions
                    if (tmp.getConnections() != null && canUseNode(tmp) &&
                        (tmp.getTile().getName().contains("RCLK_CLEL") ||
                         tmp.getTile().getName().contains("RCLK_RCLK_BRAM"))) {
                        ClockRegion crTmp = tmp.getTile().getClockRegion();
        ...
        while (!clockQueue.isEmpty()) {
            ...
            for (Wire w : currNode.getWireConnections()) {
                ...
                if (!(visitedNodes.contains(tmp))) {
                    ...
                    ClockRegion crTmp = tmp.getTile().getClockRegion();
                    if (tmp.getConnections() != null && canUseNode(tmp) && crSink.getRow() == crTmp.getRow()) {
                        if (tmp.getIntentCode() == IntentCode.NODE_GLOBAL_HDISTR &&
                            crSink.getColumn() == crTmp.getColumn()) {
    ...
    public RouteNode routeCentroidToSinkClkRegion(RouteNode clkRoute, boolean debug) {
        ClockRegion crSink = currSink.getTile().getClockRegion();
        if (debug)
            System.out.println("clkRoutestoClkRegion" + clkRoute.toString() + "->" + currSink.toString());
        clockQueue.clear();
        clockQueue.add(clkRoute);
        while (!clockQueue.isEmpty()) {
            RouteNode currNode = clockQueue.remove();
            for (Wire w : currNode.getWireConnections()) {
                if (currNode.getWireName().contains("VCC_WIRE"))
                    continue;
                    ...
                    if (tmp.getConnections() != null && canUseNode(tmp)) {
                        ...
                        if (isClockResource(tmp)) {
                            ...
                            if (crTmp.getRow() == crSink.getRow()) {
                                if ((tmp.getIntentCode() == IntentCode.NODE_GLOBAL_VDISTR ||
                                     tmp.getIntentCode() == IntentCode.NODE_GLOBAL_VROUTE) &&
                                    tmp.getTile().getTileTypeEnum() != TileTypeEnum.XIPHY_L) {
                                    RouteNode tmpVR = getHDISTRfromVHROUTE(tmp, debug);
        ...
        throw new RuntimeException("ERROR: We could not reach to a Clock Region from the Clock Route of Net " +
                                   currNet.getName());
    }
    ...
    public void routeClockTrees(boolean debug, SitePinInst currSource) {
        ...
        for (SitePinInst currPin : currNet.getPins()) {
            ...
            if (clkRegion == null) {
                ...
                if (!supressWarningsErrors)
                    System.out.println("WARNING: Failed to route clock: " + currNet.getName());
                return;
            }
            if (debug)
                printClkNodeInfo(clkRoot, "clkRoute");
            if (debug)
                printClkNodeInfo(clkRegion, "clkRegion");
            visitedNodes.clear();
            ...
            if (clkSink != null) {
                ...
                while (currPathNode.getParent() != null) {
                    if (debug)
                        System.out.println("CL: " + StringTools.makeWhiteSpace(currPathNode.getLevel()) +
                                           currPathNode.toString());
                    if (allowWireOverlap) {
                        if (usedNodes.contains(currPathNode)) {
                            conflictNodes.add(currPathNode);
                        }
                    ...
                    for (Wire w1 :
                         currPathNode.getParent().getTile().getWireConnections(currPathNode.getParent().getWire())) {
                        if (w1.getWireIndex() == currPathNode.getWire()) {
                            if (w1.isEndPIPWire() &&
                                currPathNode.getParent().getTile().equals(currPathNode.getTile())) {
                                ...
                                // This is a bidirectional wire, we need to reverse the wire0 and
                                // wire1 so Vivado can interpret the PIPs correctly
                                int wire0 = currPathNode.getParent().getWire();
                                int wire1 = currPathNode.getWire();
                                if (w1.getPIPType().isBidirectional()) {
                                    List<PIP> pipArray =
                                        currPathNode.getTile().getPIPs(currPathNode.getParent().getWire());
                                    for (PIP tp : pipArray) {
                                        if (tp.getEndWireIndex() == currPathNode.getWire()) {
                                            // TODO - This is a bidirectional PIP, Vivado always
                                            // prefers one direction
                                        }
                                    }
                                }
                                ...
                                if (debug)
                                    System.out.println("PIP-" + tmpPIP.getTile() + "/" + tmpPIP.getStartWireName() +
                                                       "->" + tmpPIP.getTile() + "/" + tmpPIP.getEndWireName());
                                pipList.add(tmpPIP);
                                break;
                            }
                        }
                    }
                    // Update the current node to the parent, this way we can traverse backwards to
                    // the source
                    currPathNode = currPathNode.getParent();
                }
                if (pathFromSinkToSwitchBox != null && pathFromSinkToSwitchBox.size() > 1) {
                ...
                throw new RuntimeException("ERROR: Did not reach the sink: " + currSink.getTile() + "/" +
                                           currSink.getWireName());
            }
        } // end of sinks
    }
    ...
    public void routeClockNet() {
        ...
        if (currNet.needsClockNetworkResources()) {
            ...
            System.out.println("NOTE: Clock net " + currNet.getName() +
                               " was not routed, further developement on clock router needed.");
            return;
        }
            ...
            if (successfulRoute) {
                ...
                if (ENABLE_RIPUP) {
                    ...
                    String switchMatrixString = switchMatrixSink != null ? switchMatrixSink.getTile().getName() + " " +
                                                                               switchMatrixSink.getWireName()
                                                                         : "null";
                    MessageGenerator.briefError("\tFAILED TO ROUTE: net: " + currNet.getName() +
                                                " inpin: " + currSinkPin.getName() + " (" +
    ...
    private boolean pruneNode(RouteNode routeNode) {
        ...
        if (usedNodes.contains(routeNode))
            return true;
        if (visitedNodes.contains(routeNode))
            return true;
        return false;
    }
    ...
    public void routePinsReEntrant(ArrayList<SitePinInst> sitePinInsts, boolean routeUnroutedNets) {
        ...
        for (SitePinInst currPin : sitePinInsts) {
            ...
            if (currPin.getNet() == null) {
                throw new RuntimeException("ERROR: Cannot route pin " + currPin.toString() +
                                           " because it is not part of a net.");
            }
    ...
    public Design routeDesign() {
        ...
        for (Net nn : design.getNets()) {
            ...
            // Consider all nets as fully routed except static nets (TODO - add support to analyze
            // all nets)
            if (currNet.getPIPs().size() > 0)
                continue;
            ...
            if (currNet.getSource() == null && !currNet.isStaticNet()) {
                ...
                if (!supressWarningsErrors)
                    MessageGenerator.briefError("WARNING: " + currNet.getName() +
                                                " does not have a source pin associated with it.");
                continue;
            }
        ...
        for (SitePinInst sink : failedRoutes) {
            ...
            if (successfulRoute) {
                ...
                String switchMatrixString = switchMatrixSink != null ? switchMatrixSink.getTile().getName() + " " +
                                                                     ...
                                                                     : "null";
                MessageGenerator.briefError("\tFAILED TO ROUTE: net: " + currNet.getName() +
                                            " inpin: " + currSinkPin.getName() + " (" + currSink.getTile().getName() +
                                            " " + currSink.getWireName() + " / " + switchMatrixString +
                                            ") on instance: " + currSinkPin.getSiteInstName());
                continue;
            }
        }
    ...
    protected static void printTimeHelper(String timedOperation, long start) {
        System.out.printf("%s %8.3fs\n", timedOperation, (System.nanoTime() - start) / 1000000000.0);
    }
    ...
    public static ArrayList<RouteNode> findInputPinFeed(SitePinInst p) {
        ...
        Tile t = site.getTile();
        if (site.isOutputPin(pinName))
            return null;
        if (ignoreInputs.contains(pinName))
            return null;
        int watchdog = 1000;
        int wire = site.getTileWireIndexFromPinName(pinName);
        if (wire == -1)
            return null; // Pin is not connected to anything (unbonded IO)
        Node node = Node.getNode(t, wire);
        RouteNode n = new RouteNode(node.getTile(), node.getWireIndex());
        Queue<RouteNode> q = new LinkedList<RouteNode>();
        q.add(n);
