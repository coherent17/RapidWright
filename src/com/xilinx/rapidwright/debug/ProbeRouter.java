...
public class ProbeRouter {
     ...
     * @param probeToTargetNets A map from probe names to desired net names (full hierarchical
     *     names).
     */
    ...
    public static void updateProbeConnections(Design d, Map<String, String> probeToTargetNets, PBlock pblock) {
        ...
        for (Entry<String, String> e : probeToTargetNets.entrySet()) {
            ...
            String parentCellInstName =
                cellInstName.contains(EDIFTools.EDIF_HIER_SEP)
            ...
            String hierInstName = cellInstName.contains(EDIFTools.EDIF_HIER_SEP)
                                      ...
                                      : "";
            EDIFHierPortInst startingPoint =
                new EDIFHierPortInst(d.getNetlist().getHierCellInstFromName(hierInstName), portInst);
            ...
            if (sinks.size() != 1) {
                System.err.println("ERROR: Currently we only support a single flip flop "
                                   + "sink for probe re-routes, found " + sinks.size() + " on " + e.getKey() +
                                   ", skipping...");
                continue;
            }
            ...
            BELPin inPin =
            c.getBEL().getPin(c.getPhysicalPinMapping(sinkFlop.getPortInst().getName())); BELPin
            rbel = inPin.getSiteConns().get(0); c.getSiteInst().addSitePIP(rbel.getBEL().getName(),
            "BYP", rbel.getName()); String siteWireName =
            rbel.getBEL().getPin("BYP").getSiteWireName(); c.getSiteInst().addCTag(destPhysNet,
            siteWireName);*/
    ...
    nextInst:
        for (EDIFHierCellInst i : candidates) {
    ...
    private static void printHelp() {
        System.out.println("USAGE: <input.dcp> <probes.txt> <output.dcp> [" + PBLOCK_SWITCH +
