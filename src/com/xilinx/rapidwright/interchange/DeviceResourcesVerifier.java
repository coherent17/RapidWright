...
public class DeviceResourcesVerifier {
    ...
    private static boolean expect(boolean gold, boolean test) {
        if (gold != test)
            throw new RuntimeException("ERROR: Device mismatch: gold=" + gold + ", test=" + test);
        return true;
    }
    ...
    private static void verifyBelPin(StructList.Reader<DeviceResources.Device.BELPin.Reader> belPins, BELPin pin,
                                     int belPinIndex) {
    ...
    public static boolean verifyDeviceResources(String devResFileName, String deviceName) throws IOException {
        ...
        for (int i = 0; i < tileCount; i++) {
            ...
            // Note: Tile.getUniqueAddress() is equivalent to the INDEX property on a Vivado Tile
            // object
            ...
            for (int j = 0; j < tile.getSites().length; j++) {
                ...
                expect(site.getSiteTypeEnum().name(), allStrings.get(siteTypeName));
                ...
                for (int k = 0; k < site.getSitePinCount(); k++) {
                    ...
                    expect(allStrings.getIndex(site.getTileWireNameFromPinName(pinName)), pinToWires.get(k));
                }
            ...
            for (int j = 0; j < pips.size(); j++) {
                ...
                boolean isBuffered20 =
                    type == PIPType.BI_DIRECTIONAL_BUFFERED20 || type == PIPType.BI_DIRECTIONAL_BUFFERED21_BUFFERED20;
                boolean isBuffered21 =
                    type == PIPType.BI_DIRECTIONAL_BUFFERED21_BUFFERED20 || type == PIPType.DIRECTIONAL_BUFFERED21;
                ...
                if (pipReader.hasPseudoCells()) {
                    ...
                    if (foundBELPins.size() > 0) {
                        throw new RuntimeException("ERROR: Found unknown BELPins " + foundBELPins + " for pseudo PIP " +
                                                   pipHelper.getPseudoPIPName() + " ");
                    }
                }
            }
        ...
        for (String libExpected :
             new String[] {EDIFTools.EDIF_LIBRARY_HDI_PRIMITIVES_NAME, LogNetlistWriter.DEVICE_MACROS_LIB}) {
        ...
        if (size > 0) {
            throw new RuntimeException("Found the following unexpected librar" + (size > 1 ? "ies" : "y") + ": " +
                                       libsFound);
        }
        ...
        for (EDIFLibrary lib : primsAndMacros.getLibraries()) {
            EDIFLibrary reference = lib.isHDIPrimitivesLibrary()
                                        ? Design.getPrimitivesLibrary(design.getDevice().getName())
            ...
            if (!cellsFound.containsAll(cellsExpected)) {
                ...
                throw new RuntimeException("Missing some cells expected in library " + lib.getName() + ": " +
                                           cellsExpected);
            }
            if (!cellsExpected.containsAll(cellsFound)) {
                cellsFound.removeAll(cellsExpected);
                throw new RuntimeException("Extra cells found in library " + lib.getName() + ": " + cellsFound);
            }
        }
            ...
            if (entry.hasParameters()) {
                ...
                if (!mapping.getFirst().equals(macroName)) {
                    throw new RuntimeException("Exception map mismatch: "
                                               + "(" + primName + "-->" + macroName +
                                               ") does not match expected mapping (" + primName + "-->" +
                                               EDIFNetlist.macroExpandExceptionMap.get(primName) + ")");
                }
        ...
        if (containsRoutingResources) {
            ConstantDefinitions.verifyConstants(allStrings, device, design, siteTypes, dReader.getConstants(),
                                                tileTypeEnumMap);
        }
    ...
    private static boolean verifySiteType(Device device, DeviceResources.Device.Reader dReader, Site site,
                                          int siteTypeIdx) {
        ...
        for (int i = 0; i < pinCount; i++) {
            ...
            if ((isInput != (dir == Direction.INPUT)) || (isOutput != (dir == Direction.OUTPUT))) {
                throw new RuntimeException("ERROR: Mismatch on site pin direction, site pin " + pinName + " for site " +
                                           site.getName());
            }
        }
        expect(highestIndexInputPin, stReader.getLastInput());
        ...
        for (int i = 0; i < site.getBELs().length; i++) {
            ...
            for (int j = 0; j < bel.getPins().length; j++) {
                ...
                DeviceResources.Device.BELPin.Reader bpReader = stBPReader.get(belPinsReader.get(j));
        ...
        for (int i = 0; i < site.getSiteWireCount(); i++) {
            ...
            for (int j = 0; j < siteWire.getPins().size(); j++) {
                DeviceResources.Device.BELPin.Reader bp = stBPReader.get(siteWire.getPins().get(j));
    ...
    static private void verifyCellBelPinMap(Map<SiteTypeEnum, List<Site>> siteMap, CellBelMapping cellBelMap,
                                            EDIFCell topLevelCell, EDIFCell cell, Design design) {
        ...
        if (!siteTypes.equals(sitesFromDev.keySet())) {
            throw new RuntimeException(String.format("Cell %s -> set of site types does not match", cell.getName()));
        }
        ...
        for (SiteTypeEnum siteType : siteTypes) {
            ...
            if (!bels.equals(belsFromDev)) {
                throw new RuntimeException(
                    String.format("Cell %s -> BELs for site type %s doesn't match", cell.getName(), siteType.name()));
            }
        }
        ...
        for (Map.Entry<SiteTypeEnum, String> possibleSite : entries) {
            ...
            for (List<String> parameters : EnumerateCellBelMapping.getParametersFor(series, cell.getName())) {
                ...
                Map<String, String> pinMappingFromDev =
                    cellBelMap.getPinMappingsP2L(cell.getName(), siteType, bel, parameterArray);
                    ...
                    if (!pinMapping.equals(pinMappingFromDev)) {
                        ...
                        for (String belPin : pinMapping.keySet()) {
                            ...
                            if (!pinMapping.get(belPin).equals(pinMappingFromDev.get(belPin))) {
                                System.out.printf(" - %s != %s\n", pinMapping.get(belPin),
                                                  pinMappingFromDev.get(belPin));
                            }
                        }

                        throw new RuntimeException(
                            String.format("Cell %s -> BEL pins for site type %s and parameters %s doesn't match",
    ...
    static private void verifyParameterDefinitiosn(StringEnumerator allStrings, DeviceResources.Device.Reader dReader,
                                                   Design design) {
        EDIFLibrary prims = Design.getPrimitivesLibrary(design.getDevice().getName());
        ...
        for (EDIFCell cell : prims.getCells()) {
            ...
            Map<String, VivadoProp> defaultCellProperties =
                Design.getDefaultCellProperties(design.getDevice().getSeries(), cellTypeName);
        ...
        for (CellParameterDefinition.Reader cellParamDef : paramDefs.getCells()) {
            ...
            if (!cellsWithParameters.contains(cellType)) {
                throw new RuntimeException(
                    String.format("Cell %s has parameters in DeviceResources, but not in RapidWright?", cellType));
            }

            Map<String, VivadoProp> defaultCellProperties =
                Design.getDefaultCellProperties(design.getDevice().getSeries(), cellType);
            ...
            for (ParameterDefinition.Reader paramDef : cellParamDef.getParameters()) {
                ...
                if (!defaultCellProperties.containsKey(paramName)) {
                    throw new RuntimeException(String.format(
                        "Cell %s has parameter %s in DeviceResources, but not in RapidWright?", cellType, paramName));
                }
                if (!paramDef.getDefault().isTextValue()) {
                    throw new RuntimeException(
                        String.format("Cell %s parameter %s default is not a textValue", cellType, paramName));
                }
                ...
                if (expected != paramDef.getFormat()) {
                    throw new RuntimeException(String.format("Expected ParameterFormat %s got %s", expected.name(),
                                                             paramDef.getFormat().name()));
                }
            }
        }
    }

    static private void verifyCellBelPinMaps(StringEnumerator allStrings, DeviceResources.Device.Reader dReader,
                                             Design design) {
        EDIFLibrary prims = Design.getPrimitivesLibrary(design.getDevice().getName());
    ...
    static private void verifyPackages(StringEnumerator allStrings, DeviceResources.Device.Reader dReader,
                                       Device device) {
    ...
    static private void verifyCellInversions(Device device, DeviceResources.Device.Reader dReader,
                                             Set<Unisim> unisimsExpected) {
