...
public class PhysNetlistReader {
        ...
        for (int i = 0; i < placementCount; i++) {
            ...
            if (physType == PhysCellType.LOCKED) {
                ...
                if (macroPrims.containsCell(cellType)) {
                    throw new RuntimeException("ERROR: Placement for macro primitive " + cellType + " (instance " +
                                               cellName + ") is "
                ...
                if (bel == null) {
                    ...
                    throw new RuntimeException("ERROR: The placement specified on BEL " + siteName + "/" + belName +
                                               " could not be found in the target "
                                               + "device.");
                }
                if (bel.getBELType().equals("HARD0") || bel.getBELType().equals("HARD1")) {
                    String siteName = strings.get(placement.getSite());
                    throw new RuntimeException("ERROR: The placement specified on BEL " + siteName + "/" +
                ...
                if (existingCell != null) {
                    String siteName = strings.get(placement.getSite());
                    throw new RuntimeException("ERROR: Cell \"" + cellName + "\" placement on BEL " + siteName + "/" +
                                               belName + " conflicts with previously placed cell \"" +
                                               existingCell.getName() + "\".");
                }
            ...
            for (int j = 0; j < pinMapCount; j++) {
                ...
                if (c == null) {
                    if (otherBELLocs.remove(pinMapping.getBel())) {
                        BEL bel = siteInst.getBEL(belName);
                        if (bel == null) {
                            throw new RuntimeException("ERROR: Couldn't find BEL " + belName + " in site " +
                                                       siteInst.getSiteName() + " of type " +
                        ...
                        throw new RuntimeException("ERROR: Missing BEL location in other BEL list: " + belName +
                                                   " on for pin mapping " + belPinName + " -> " + cellPinName);
                    }
                }
                ...
                if (c.getLogicalPinMapping(belPinName) != null && pinMapping.hasOtherCell()) {
                    ...
                    c.addAltPinMapping(belPinName, new AltPinMapping(cellPinName, strings.get(otherCell.getMultiCell()),
                                                                     strings.get(otherCell.getMultiType())));
                } else {
                    if (c.getBEL().getPin(belPinName) == null) {
                        System.err.println("WARNING: On cell " + c.getName() + ", a logical pin '" + c.getType() + "." +
                                           cellPinName + "' is being mapped on to a BEL pin '" + c.getBELName() + "." +
                                           belPinName + "' that does not exist. "
        ...
        if (VALIDATE_MACROS_PLACED_FULLY) {
            ...
            for (Cell c : design.getCells()) {
                ...
                if (cellType != null && macroPrims.containsCell(cellType)) {
                    ...
                    if (checked.contains(parentHierName))
                        continue;
                    List<String> missingPlacements = null;
                    List<String> childrenNames = macroLeafChildren.computeIfAbsent(
                        cellType.getName(), (k) -> EDIFTools.getMacroLeafCellNames(cellType));
                    // for (EDIFCellInst inst : cellType.getCellInsts()) { // TODO - Fix up loop
                    // list
        ...
        switch (netReader.getType()) {
            case GND:
                if (!netName.equals(Net.GND_NET)) {
                    throw new RuntimeException("ERROR: Invalid GND Net " + netName + ", should be named " +
                                               Net.GND_NET);
                }
                return NetType.GND;
            case VCC:
                if (!netName.equals(Net.VCC_NET)) {
                    throw new RuntimeException("ERROR: Invalid VCC Net " + netName + ", should be named " +
                                               Net.VCC_NET);
                }
                return NetType.VCC;
            default:
    ...
    protected void readNullNet(PhysNetlist.Reader physNetlist) {
        ...
        for (int k = 0; k < stubCount; k++) {
            ...
            sitePIPSiteInst.addSitePIP(strings.get(spReader.getBel()), strings.get(spReader.getPin()));
        }
    }
    ...
    private void readRouteBranch(Set<Wire> stubWires, RouteBranch.Reader branchReader, Net net,
        ...
        switch (segment.which()) {
            case PIP: {
                ...
                if (tile == null) {
                    ...
                    throw new RuntimeException("ERROR: Tile " + strings.get(pReader.getTile()) + " for pip from wire " +
                                               wire0 + " to wire " + wire1 + " not found.");
                }
                ...
                if (pip == null) {
                    String wire0 = strings.get(pReader.getWire0());
                    String wire1 = strings.get(pReader.getWire1());
                    System.err.println("WARNING: PIP for tile " + strings.get(pReader.getTile()) + " from wire " +
                                       wire0 + " to wire " + wire1 + " not found;"
                            ...
                            if (routeThruLutInput != null) {
                                ...
                                if (routeThruCell == null) {
                                    ...
                                    if (siteInst.getCell(routeThruLutInput.getBEL()) != null) {
                                        throw new RuntimeException("Routethru inferred for " + siteInst.getSiteName() +
                                    ...
                                    routeThruCell.addPinMapping(routeThruLutInput.getName(),
                                                                belCell.getLogicalPinMapping(belPinName));
                                }

                                String physicalPin = routeThruLutInput.getName();
                                String logicalPin = belCell.getLogicalPinMapping(belPinName);
                                if (routeThruCell.getSiteInst() != siteInst || !routeThruCell.isRoutethru() ||
    ...
    private static void checkNetTypeFromCellNet(Map<String, PhysNet.Reader> cellPinToPhysicalNet, EDIFHierNet net,
                                                List<String> strings) {
                    ...
                    if (innerNet != null) {
                        ...
                        if (physNet != null) {
                            if (physNet.getType() != PhysNetlist.NetType.VCC &&
                                physNet.getType() != PhysNetlist.NetType.GND) {
                                throw new RuntimeException(
                                    String.format("ERROR: Net %s connected to cell pin %s should "
                                                      + "be VCC or GND but is %s",
    ...
    private void mapBelPinsToPhysicalNets(Map<String, PhysNet.Reader> belPinToPhysicalNet, PhysNet.Reader netReader,
        ...
        if (segment.which() == RouteSegment.Which.BEL_PIN) {
            PhysBelPin.Reader bpReader = segment.getBelPin();
            belPinToPhysicalNet.put(strings.get(bpReader.getSite()) + "/" + strings.get(bpReader.getBel()) + "/" +
    ...
    protected void checkConstantRoutingAndNetNaming(PhysNetlist.Reader physNetlist) {
        ...
        for (PhysNet.Reader physNet : physNetlist.getPhysNets()) {
            ...
            if (!netNames.add(physNet.getName())) {
                throw new RuntimeException(String.format("ERROR: Net %s appears in physical netlist more than once?",
                                                         strings.get(physNet.getName())));
            }
        ...
        for (CellPlacement.Reader placement : physNetlist.getPlacements()) {
            for (PinMapping.Reader pinMap : placement.getPinMap()) {
                String key = strings.get(placement.getSite()) + "/" + strings.get(pinMap.getBel()) + "/" +
                             strings.get(pinMap.getBelPin());
                PhysNet.Reader net = belPinToPhysicalNet.get(key);
                if (net != null) {
                    PhysNet.Reader oldNet = cellPinToPhysicalNet.put(
                        strings.get(placement.getCellName()) + "/" + strings.get(pinMap.getCellPin()), net);
