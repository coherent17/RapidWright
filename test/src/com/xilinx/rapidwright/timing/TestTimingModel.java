...
public class TestTimingModel {
    ...
    public void
    checkFindReferenceTile(String deviceName, int expectedTileX, int expectedTileY, int expectedCol, int expectedRow) {
        TimingModel model = new TimingModel(Device.getDevice(deviceName));
        model.build();
        Tile tile = model.getRefIntTile();
        System.out.println(deviceName + " " + tile.getTileXCoordinate() + " " + tile.getTileYCoordinate() + " " +
